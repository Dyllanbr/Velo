...
- Bypass e redirects no navegador: a leitura protegida do marcador e o ramo do aplicativo usam `fetch` nativo do Node, restrito à origem exata do deployment e sem credenciais na URL. Os headers protegidos não são enviados por `route.fetch`/`APIRequestContext`, cuja instrumentação pode reter detalhes no relatório de falha. O transporte usa `redirect: 'manual'`; respostas 3xx do aplicativo são registradas de forma saneada e abortadas antes de chegar ao navegador. Respostas aceitas são entregues por `route.fulfill`. Erros de transporte omitem headers e causa. O ciclo de vida mantém o bloqueio de rede instalado, espera os handlers pendentes e aborta novas requisições durante o encerramento. O anexo `blocked-app-redirect.json` conserva apenas origem/status/ação, sem Location ou headers; chamadas Supabase não recebem bypass. A correção possui validação local e laboratório com chave fictícia, mas isso não substitui a execução remota correspondente ao SHA final.
