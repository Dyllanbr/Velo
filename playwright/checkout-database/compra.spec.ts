...
test.describe('compra preview: limpar antes e manter o pedido próprio', () => {
  ...
  for (const round of [1, 2]) {
    test(`rodada ${round}: compra à vista pela interface e confirmação SQL`, async ({ page, context }, testInfo) => {
      ...
      const lastWriteStart = performance.now() + writeWindow;
      let active = true;
      page.once('close', () => { active = false; });
      const assertActive = () => {
        if (!active || performance.now() >= lastWriteStart) throw new Error('Checkout write window ended; no new operation allowed.');
      };
      // Marker + app bundle are checked before opening a Pool or deleting fixture data.
      const deployment = await verifyCheckoutDeployment(preview);
