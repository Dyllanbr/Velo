...
export function createOrderLookupActions(page: Page) {
  ...
  return {
    ...
    async open() {
      ...
      await page.getByTestId('header-nav').getByRole('link', {
    ...
    async validateOrderDetails(order: OrderDetails) {
      ...
      await expect(page.getByTestId(`order-result-${order.number}`)).toMatchAriaSnapshot(String.raw`
        - paragraph: Pedido
    ...
    async validateStatusBadge(
      ...
      const statusBadge = page.getByTestId(`order-result-${orderNumber}`).getByRole('status');
